<template>
  ...
  <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 max-w-2xl">
    <!-- Conversion Flow -->
    <div v-if="widget.require && widget.require.length > 0" class="flex items-center justify-between">
      <!-- Required Items -->
      ...
      <!-- Fabricator -->
      <NuxtLink
        :to="`/entry/${widget.fabricator.link_id}`"
        class="flex flex-col items-center p-2 bg-white rounded-lg shadow-sm border hover:bg-gray-50 transition-colors duration-200 no-underline"
      >
        <img 
        ...
        />
        <span class="text-xs text-center text-gray-700" v-html="widget.fabricator.text1"></span>
        <span v-if="widget.fabricator.text2" class="text-xs text-gray-500" v-html="widget.fabricator.text2"></span>
      </NuxtLink>
    ...
    <!-- Layout when no required items -->
    <div v-else class="flex items-center">
      <!-- Fabricator (right-aligned in left half) -->
      ...
      <!-- Produced Items (left-aligned in right half) -->
      <div class="flex-1 flex gap-2 pl-4">
        <NuxtLink
          v-for="(item, index) in widget.produce" 
          :key="`produced-${index}`"
          :to="`/entry/${item.link_id}`"
